...
class MethodConversionHandler:
    ...
    def convert_to_simple(cls, categorized_method: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        psp_units = [unit for unit in units if unit.get("categories", {}).get("type") == "psp"]

        # Check for atomic orbital unit
        ao_unit = next((unit for unit in units if unit.get("categories", {}).get("type") == "ao"), None)

        # Check for regression unit
        regression_unit = next((unit for unit in units if "regression" in unit.get("name", "")), None)
