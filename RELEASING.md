...
`github:mat3ra/mode#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/mode/releases/download/wip-<short-sha>/mode.tgz
```
...
```bash
npm run mat3ra:install -- mode wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/mode@<release-url>` with
