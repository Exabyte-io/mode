...
class DFTModel extends _model.Model {
  ...
  get groupSlug() {
    return [this._application.shortName, this.type, this.subtype, this.functional.slug, this.refiners.map(o => o.slug).join("+"), this.modifiers.map(o => o.slug).join("+")].join(":").replace("::", ":").replace(/:$/, "");
  }
  ...
  get allFunctionals() {
    return this.treeBranchForSubType.functionals.map(x => (0, _tree.treeSlugToNamedObject)(x));
  }
  /**
